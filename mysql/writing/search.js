...
var write = function(res, req, id) {
	...
	if (id.option == '0') {
		...
		sql = 'SELECT user,title,modificationtime,id,label,look,comment FROM article where isDelect=0 and `option`=' + id.option +
			' order by modificationtime is null, modificationtime ASC LIMIT  ' + pageIndex + ',' + pageSize;

	} else {
		count = 'select count(*) from article where isDelect=0 and user="' + id.user + '"';
		sql = 'SELECT user,title,modificationtime,id,label,look,comment FROM article where isDelect=0 and `user`="' + id.user +
			'" order by modificationtime is null, modificationtime ASC LIMIT  ' + pageIndex + ',' + pageSize;
	}
